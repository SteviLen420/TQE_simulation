...
MASTER_CTRL = {
    ...
    "ENTROPY_NOISE_SCALE": 0.05,     # base fluctuation strength (smaller → smoother)
    "ENTROPY_NOISE_SPIKE": 0.1,     # occasional spike magnitude (set lower to avoid big jumps)
    "ENTROPY_SPIKE_PROB": 0.0001,     # probability of a spike at each step (reduce for smoother curves)
    "ENTROPY_SMOOTH_WIN": 25,        # moving average window size (increase to smooth global entropy line) 
...
def simulate_entropy_universe(E, I,
    ...
    for step in range(steps):
        ...
        base_noise = np.convolve(base_raw_noise, np.ones(41)/41, mode="same")
        ...
        for r in range(num_regions):
            ...
            indiv_noise = np.convolve(indiv_raw, np.ones(41)/41, mode="same")
            ...
            if rng.random() < MASTER_CTRL["ENTROPY_SPIKE_PROB"]:
                ...
                noise += np.convolve(spike, np.ones(41)/41, mode="same")
...
pd.DataFrame({"time": np.arange(len(best_global_entropy)),
              "global_entropy": best_global_entropy}).to_csv(
