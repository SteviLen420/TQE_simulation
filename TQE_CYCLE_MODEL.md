...
- **Renormalise** — $\displaystyle \tilde{P}_{k+1}(\psi) = \frac{P_{k+1}(\psi)}{\int_\Psi P_{k+1}(\phi)\, d\phi}$.
...
- **Hard projection** — $\displaystyle P_0^+(\psi) = \frac{P_0^-(\psi)\, \mathbf{1}_{X(\psi)\in G}}{\int P_0^-(\phi)\, \mathbf{1}_{X(\phi)\in G}\, d\phi}$.
...
1. **Entropic reweighting** — $\displaystyle \Pi[P](\psi) = \frac{e^{-\gamma S[P]} P(\psi)}{\int e^{-\gamma S[P]} P(\phi)\, d\phi}$ with $\displaystyle S[P] = -\int P \ln P\, d\psi$.

2. **Perturbative noise injection** — $\displaystyle \Pi[P](\psi) = P(\psi) + \varepsilon\, \eta(\psi)$, where $\eta$ is a zero-mean random field.

3. **Cyclic rescaling** — $\displaystyle \Pi[P](\psi) = \frac{P(\psi/\alpha)}{\alpha}$ to capture geometric contraction/expansion at aeon boundaries.
