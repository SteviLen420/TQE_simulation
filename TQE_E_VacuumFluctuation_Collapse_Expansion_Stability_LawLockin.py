...
# ======================================================
# Monte Carlo: Stability + Law lock-in 
...
pd.DataFrame({"E": E_vals,"Stable": stables}).to_csv(
    os.path.join(SAVE_DIR, "stability.csv"), index=False)
    
# ======================================================
# Stability summary (counts + percentages) - unified
# ======================================================
...
# ======================================================
plt.figure()
plt.hist([e for e in law_epochs if e >= 0], bins=50, color="blue", alpha=0.7)
...
summary = {
    "simulation": {"total_universes": N,
                   "stable_fraction": float(np.mean(stables)),
                   "unstable_fraction": 1.0 - float(np.mean(stables))},
    "superposition": {"mean_entropy": float(np.mean(S)), "mean_purity": float(np.mean(P))},
    "collapse": {"mean_X": float(np.mean(X_vals)), "std_X": float(np.std(X_vals))},
    "law_lockin": {
