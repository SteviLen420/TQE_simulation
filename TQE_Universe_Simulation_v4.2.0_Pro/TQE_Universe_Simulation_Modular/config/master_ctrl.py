...
MASTER_CTRL = {
    ...
    "PLANCK_DATA_PATH": os.path.join(
        os.path.expanduser("~"),
        "Desktop",
        "CMB_MAPS",
        "planck_data",
        "COM_PowerSpect_CMB-TT-full_R3.01.txt"
    ),  # Absolute path to Planck C_ell
    ...
    "CMB_NPIX": 128,           # CMB map pixel count (can be reduced for memory constraints)
    ...
    "DRIVE_BASE_DIR":        None,    # Base directory for single run modes (None = use SIMULATION_RUNS/universe)
    "REDUCE_MEMORY_USAGE":   False,   # Reduce memory usage (set to True if memory constrained)
