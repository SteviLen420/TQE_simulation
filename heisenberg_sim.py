...
def expectation_and_sigma(x, density, dx):
    """Compute expectation value and standard deviation for density over x."""
...
def sigma_p_from_fft(psi, dx):
    ...
    dens_p /= np.sum(dens_p) * (p[1]-p[0])  # normalize in p-space
    Ep, sigp = expectation_and_sigma(p, dens_p, (p[1]-p[0]))
...
plt.xlim(-30, 30)
plt.tight_layout()
plt.savefig(f"{outdir}/position_density_sigma{sigma_x0:.1f}.png", dpi=200)
...
plt.xlim(-3, 3)
plt.tight_layout()
plt.savefig(f"{outdir}/momentum_density_sigma{sigma_x0:.1f}.png", dpi=200)
...
for sx in sigmas:
    ...
    psi = normalize(psi, dx)
    dens_x = np.abs(psi)**2
    _, sigx = expectation_and_sigma(x, dens_x, dx)
    _, sigp, _ = sigma_p_from_fft(psi, dx)
    rows.append([sx, sigx, sigp, sigx*sigp])
...
plt.legend()
plt.tight_layout()
plt.savefig(f"{outdir}/uncertainty_product.png", dpi=200)
...
for _ in range(steps):
    ...
    # back to p-space for the other half step
    Psi = fftshift(fft(np.fft.ifftshift(psi_t))) * dx/np.sqrt(2*np.pi)
    Psi *= phase_half
    psi_t = fftshift(ifft(np.fft.ifftshift(Psi))) * (np.sqrt(2*np.pi)/dx)
    ...
    dens = np.abs(psi_t)**2
    _, sx = expectation_and_sigma(x, dens, dx)
    sigx_t.append(sx)
...
plt.tight_layout()
plt.savefig(f"{outdir}/free_spreading_sigma_x_t.png", dpi=200)
