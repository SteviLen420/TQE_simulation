...
MASTER_CTRL = {
    ...
    "OUTPUTS": {
        ...
        # Local/Colab/Cloud targets (mounting logic handled elsewhere)
        "local": {
            "base_dir": None,
            "fig_subdir": "figs",
...
def _deep_merge(base, override):
    """Right overrides left; recursively merges dicts."""
...
# ===================================================================================
# Profile resolution (NO imports to other TQE modules here)
# ===================================================================================
SELECTED_PROFILE = os.environ.get("TQE_PROFILE", "demo")

def resolve_profile(profile_name: str):
    """Build ACTIVE by merging MASTER_CTRL with the selected profile overrides."""
    base = deepcopy(MASTER_CTRL)
    profs = base.pop("PROFILES", {})
    chosen = profs.get(profile_name, {})
    active = _deep_merge(base, chosen)

    # Optional: annotate which profile was selected
    meta = dict(active.get("META", {}))
    meta["selected_profile"] = profile_name
    active["META"] = meta

    # Reproducibility env caps
    repro = active.get("REPRO", {})
    if repro.get("use_strict_seed"):
        for k, v in repro.get("env_thread_caps", {}).items():
            os.environ[k] = str(v)

    # Optional run-id tag with profile name
    if active.get("OUTPUTS", {}).get("tag_profile_in_runid", False):
        os.environ["TQE_PROFILE_TAG"] = profile_name

    return active

# Final merged configuration for the whole pipeline
