...
1. **First law** — $\Delta U = Q - W$ with $f(E,I) = \exp[(Q - W)/\langle E \rangle]$, ensuring positivity for any magnitude of $Q - W$.

2. **Second law** — $\Delta S \ge 0$, $S = -k_B \sum_\psi P(\psi) \ln P(\psi)$, and $f(E,I) = \exp(\Delta S / k_B)$, i.e., entropy growth translates directly into exponential weighting.

3. **Third law** — $\lim_{T \to 0} S = \text{const.} \Rightarrow f(E,I) \to 1$ because fluctuations vanish near absolute zero.
...
Potential $U(r) = -G m_1 m_2 / r$ yields $f(E,I) = \exp\!\left[-U(r)/(k_B T)\right]$, highlighting that deeper gravitational wells receive higher probability weight at finite temperature.
...
1. **Mass–energy equivalence** — $E = mc^2 \Rightarrow f(E,I) = \frac{E}{mc^2} = 1$.

2. **Relativistic dispersion** — $E^2 = (pc)^2 + (mc^2)^2 \Rightarrow f(E,I) = \frac{E}{mc^2} = \sqrt{1 + (p/mc)^2}$.
...
The Maxwell equations enforce $U(1)$ gauge symmetry. The coupling assigns weights to static configurations via $f(E,I) = \exp\!\left[-q\, \phi(r)/(k_B T)\right]$, with $\phi(r)$ the electrostatic potential. High-energy field configurations are exponentially suppressed, while the vacuum energy $E$ still sets the background through the master coupling.
...
1. **QCD**: With Cornell potential $V(r) \approx -(4/3)\, \alpha_s \hbar c / r + k r$, the weighting $f(E,I) = \exp[-V(r)/(k_B T)]$ captures confinement by penalising long flux tubes.
...
1. **Hubble law** — $v = H_0 d \Rightarrow f(E,I) = \exp(\beta H_0 d)$, a positive, scale-dependent weighting.

2. **Friedmann equation** — $\left(\frac{\dot{a}}{a}\right)^2 = \frac{8\pi G}{3} \rho - \frac{k}{a^2} + \frac{\Lambda}{3}$. Replacing $\rho$ or $\Lambda$ with $f \rho$ or $f \Lambda$ ties microscopic selection to macroscopic expansion histories.
