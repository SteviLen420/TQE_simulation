...
At the first tier I deliberately declare the neutral setting. 

Energy conservation, $\Delta E=0$, is my definition of $f^{(1)}_{\text{eff}}=1$: when the average energy before and after a step matches, the kernel has nothing interesting to say. The same goes for linear and angular momentum; $\vec{p}_{\text{tot}}$ and $\vec{L}_{\text{tot}}$ remain constant precisely when no new information enters the system.
