...
def run_multi_i_parameter_analysis(i_definitions: list = None, pipeline_variants: list = None) -> dict:
    ...
    # Create master results directory on Desktop
    timestamp = time.strftime('%Y%m%d_%H%M%S')
    desktop_path = os.path.join(os.path.expanduser("~"), "Desktop")
    modular_results_dir = os.path.join(desktop_path, "TQE_Universe_Simulation_Modular_Results")
    
    master_save_dir = os.path.join(modular_results_dir, f"COMPARATIVE_ANALYSIS_{timestamp}")
...
def run_single_i_parameter_mode(i_definition: str = "kl_shannon", pipeline_variant: str = "full") -> dict:
    ...
    # Create directory structure for single I parameter on Desktop
    desktop_path = os.path.join(os.path.expanduser("~"), "Desktop")
    modular_results_dir = os.path.join(desktop_path, "TQE_Universe_Simulation_Modular_Results")
    ...
    single_i_dir = os.path.join(modular_results_dir, pipeline_name)
...
def run_pipeline(config_override: dict = None, run_id_override: str = None) -> dict:
    ...
    if preflight_report["status"] == "error":
        raise RuntimeError(
            "Preflight diagnostics reported blocking issues. "
            "Resolve them before running the pipeline."
        )
    ...
    postrun_report = run_postrun_diagnostics(ctx, summary)
...
if __name__ == "__main__":
    ...
    # Create run-mode-specific directory WITH TIMESTAMP on Desktop
    timestamp = time.strftime("%Y%m%d_%H%M%S")
    ...
    os.makedirs(modular_results_dir, exist_ok=True)

    # Create run-mode-specific subdirectory WITH TIMESTAMP (prevents overwriting)
    mode_dir = os.path.join(modular_results_dir, f"TQE_Universe_Simulation_{run_mode}_{timestamp}")
    ...
    # ===================================================================
        ...
        for idx, i_def in enumerate(ALL_I_DEFINITIONS):
            print(f"\n{'─'*70}")
            print(f"E+I Run {idx+1}/11: {i_def}")
            print(f"{'─'*70}")
