...
# ===========================================
# Author: Stefan Len
# Purpose: Monte Carlo simulation with Goldilocks
# ===========================================
...
# ======================================================
# 1) Information parameter (I) via Shannon entropy
# ======================================================
def sample_information_param(dim=8):
    psi = qt.rand_ket(dim)  # csak egy kvantumállapot kell
    p = np.abs(psi.full().flatten())**2
    p /= p.sum()
    eps = 1e-12
    H = -np.sum(p * np.log(p + eps))  # Shannon-entrópia
    H_norm = H / np.log(len(p))       # normalizálás: 0 ≤ H_norm ≤ 1
    return H_norm
